                ...
                <div class="">
                    <table class="datatable table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?php echo $sl++; ?></td>
                                <td><img src="<?php echo base_url(!empty($value->image) ? $value->image : 'assets/img/icons/default.jpg'); ?>"
                                ...
                                <td><?php echo (($value->status == 1) ? display('active') : display('inactive')); ?>
                                </td>
                                <td>
                                    <?php if ($value->is_admin == 0) { ?>
                                    <?php if ($this->permission->check_label('add_user')->access()) { ?>
                                    <a href="<?php echo base_url("dashboard/user/form/$value->id") ?>"
                                        class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left"
                                        title="Update">
                                        <i class="fa fa-pencil" aria-hidden="true"></i>
                                    </a>
                                    <?php }
                                            if ($this->permission->check_label('manage_users')->delete()->access()) { ?>
                                    <a href="<?php echo base_url("dashboard/user/delete/$value->id") ?>"
                                    ...
                                    <button class="btn btn-info btn-sm"
                                        title="<?php echo display('admin') ?>"><?php echo display('admin') ?></button>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php } ?>
